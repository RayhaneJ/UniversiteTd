...
public class App {
    ...
    public static final Function<Annee, Stream<Matiere>> matieresA = a -> a.etudiants()
            .stream().flatMap(e -> e.notes().keySet().stream());

    // matières d'un étudiant
    public static final Function<Etudiant, Stream<Matiere>> matieresE = e -> e.notes().keySet().stream();

    // transformation d'une Entry en une Paire
    public static final Function<Entry<Matiere, Integer>, Paire<Matiere, Integer>>
    entry2paire = entry -> new Paire<>(entry.getKey(), entry.getValue());

    // matières coefficientées d'un étudiant (version Paire)
    public static final Function<Etudiant, Stream<Paire<Matiere, Integer>>>
    matieresCoefE = e -> e.notes().entrySet().stream()
            .map(v -> new Paire<>(v.getKey(), v.getValue().intValue()));

    // accumulateur pour calcul de la moyenne
    // ((asomme, acoefs), (note, coef)) -> (asomme+note*coef, acoef+coef)
    public static final BinaryOperator<Paire<Double, Integer>> accumulateurMoyenne = (x1, x2)
            -> new Paire<>(x1.fst + x2.fst * x2.snd, x1.snd + x2.snd);
    ...
    notesPonderees = e -> e.annee().ues().stream().flatMap(ues -> ues.ects())
